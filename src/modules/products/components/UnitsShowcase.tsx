...
export const UnitsShowcase = React.memo<UnitsShowcaseProps>(({
  units,
  isLoading = false,
  onEdit,
  onView,
  ...
  if (units.length === 0) {
    return (
      <div className="text-center py-5">
        <div 
          className="bg-primary-subtle rounded-circle mx-auto mb-4 d-flex align-items-center justify-content-center"
          style={{ width: '120px', height: '120px' }}
        >
          <i className="bi bi-rulers text-primary" style={{ fontSize: '3rem' }} />
        </div>
        ...
        {units.map((unit) => (
          <div key={unit.id} className="col-lg-6">
            <div className="card h-100 shadow-lg border-0 hover-transform-scale transition-all">
              ...
              <div 
                className="card-img-top bg-gradient position-relative overflow-hidden"
                style={{ height: '400px' }}
              >
                {/* Background Pattern */}
              ...
              <div className="card-body d-flex flex-column">
                {/* Header */}
                <div className="mb-3">
                  <h4 className="fw-bold mb-2">{unit.name}</h4>
                  {unit.unitType && (
                    <div className="mb-2">
...
UnitsShowcase.displayName = 'UnitsShowcase'