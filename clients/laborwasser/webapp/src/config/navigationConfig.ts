/**
 * Labor Wasser de Mexico - Sidebar navigation config.
 *
 * Each tenant repo ships its own navigationConfig because what modules
 * are visible per role is a tenant decision. The hook `useNavigation`
 * and the structural types live in @lwm/ui; the data lives here.
 *
 * Initially cloned from apps/template to have a functional starting
 * point; adjust groups/items when AlphaLab decides which modules to
 * expose (e.g. drop system-health or discount-rules if the operation
 * is simpler than LaborWasser's).
 *
 * Permission logic: empty array = visible to anyone in the audience.
 * Non-empty uses OR logic — user with ANY of the permissions sees the
 * item. Admin/god bypasses all checks.
 */
