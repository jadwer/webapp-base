'use client'

import React, { useState } from 'react'
import { useRouter } from 'next/navigation'
import CFDIInvoiceForm from '@/modules/billing/components/CFDIInvoiceForm'
import { useCFDIInvoice, useCFDIInvoicesMutations } from '@/modules/billing'
import type { CFDIInvoiceFormData } from '@/modules/billing/types'

interface EditInvoicePageProps {
  params: Promise<{
    id: string
  }>
}

export default function EditInvoicePage({ params }: EditInvoicePageProps) {
  const resolvedParams = React.use(params)
  const router = useRouter()
  const { invoice, isLoading } = useCFDIInvoice(resolvedParams.id)
  const { updateInvoice } = useCFDIInvoicesMutations()
  const [isSubmitting, setIsSubmitting] = useState(false)

  const handleSubmit = async (data: CFDIInvoiceFormData) => {
    setIsSubmitting(true)
    try {
      await updateInvoice(resolvedParams.id, data)
      router.push('/dashboard/billing/invoices')
    } finally {
      setIsSubmitting(false)
    }
  }

  const handleCancel = () => {
    router.back()
  }

  if (isLoading) {
    return (
      <div className="container-fluid py-4">
        <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
          <div className="spinner-border text-primary" role="status">
            <span className="visually-hidden">Cargando...</span>
          </div>
        </div>
      </div>
    )
  }

  if (!invoice) {
    return (
      <div className="container-fluid py-4">
        <div className="alert alert-warning">
          <i className="bi bi-exclamation-triangle me-2" />
          Factura no encontrada
        </div>
        <button className="btn btn-secondary" onClick={handleCancel}>
          Volver
        </button>
      </div>
    )
  }

  if (invoice.status !== 'draft') {
    return (
      <div className="container-fluid py-4">
        <div className="alert alert-warning">
          <i className="bi bi-exclamation-triangle me-2" />
          Solo se pueden editar facturas en estado borrador
        </div>
        <button className="btn btn-secondary" onClick={handleCancel}>
          Volver
        </button>
      </div>
    )
  }

  return (
    <div className="container-fluid py-4">
      <div className="row">
        <div className="col-12">
          {/* Header */}
          <div className="d-flex align-items-center mb-4">
            <button
              type="button"
              onClick={handleCancel}
              className="btn btn-link p-0 me-3 text-decoration-none"
              title="Volver"
            >
              <i className="bi bi-arrow-left fs-4 text-muted" />
            </button>
            <div>
              <h1 className="h3 mb-0 fw-bold">
                <i className="bi bi-pencil-square text-warning me-2" />
                Editar Factura {invoice.series}-{invoice.folio}
              </h1>
              <p className="text-muted mb-0">Modificar los datos de la factura</p>
            </div>
          </div>

          {/* Form */}
          <div className="row justify-content-center">
            <div className="col-lg-10 col-xl-8">
              <div className="card shadow-sm border-0">
                <div className="card-body p-4">
                  <CFDIInvoiceForm
                    invoice={invoice}
                    isLoading={isSubmitting}
                    onSubmit={handleSubmit}
                    onCancel={handleCancel}
                  />
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}
